...
#
# Print the `PYEOF` heredoc of <script> whose body mentions <needle>. Both scripts
# embed the posthook handling as python, and running the embedded code is the only
# way to assert what it does rather than what it looks like.
...
#
# Prints the value, or `<absent>` when the key is not there at all - which is what
# uninstall leaves behind when a hook was only ever ours.
json_get() {
    python3 - "$@" <<'PYEOF'
import json, sys
...
test_reinstalling_keeps_a_hook_that_is_not_ours() {
    ...
    # The pattern used to be `sudo <anything> --posthook`, which is not a command this
    # installer can claim: reinstalling would have taken a user's own hook out of
    # cli.json without saying so.
    local dir
    dir="$(new_tmpdir)"
...
test_the_saved_theme_selection_is_named_the_same_twice() {
    # 05-sddm-theme.sh saves what /etc/sddm.conf had before it wrote its own theme
    # there and uninstall.sh puts it back. Two spellings of the path mean the user's
    # selection is never restored, and nothing else would notice.
...
test_the_login_wallpaper_copy_has_a_directory_of_its_own() {
    # The greeter's wallpapers directory is shared with KDE's Login Screen settings
    # module. Removing everything in it that is not the current wallpaper, which is how
    # the copy is kept to one file, would delete the pictures a user picked there.
